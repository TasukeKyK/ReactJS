import { Pokemon } from "./app/models/Interface.js";
...
$albumPokemon?.insertBefore($btn, $albumPokemon?.firstElementChild); // en el elemento main insertamos despues del sectión el botón
...
*/
const modifyElement = (pokemon: Pokemon) => {
  const queryClass = [
    ".pokemon-card__img",
  ...
  // Tipar variable
  const $elements = queryClass.map((className) =>
